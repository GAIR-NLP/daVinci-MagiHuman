...
$LAUNCH_PREFIX torchrun ${DISTRIBUTED_ARGS} inference/pipeline/entry.py ${MAGI_COMPILER_OFFLOAD_ARGS} \
  ...
  --sr_width "${SR_WIDTH}" \
  --sr_height "${SR_HEIGHT}" \
  --output_path "output_example_sr_540p_ti2v_${BR_WIDTH}x${BR_HEIGHT}_${SR_WIDTH}x${SR_HEIGHT}_$(date '+%Y%m%d_%H%M%S')" \
  2>&1 | tee "log_example_sr_540p_ti2v_${BR_WIDTH}x${BR_HEIGHT}_${SR_WIDTH}x${SR_HEIGHT}_$(date '+%Y%m%d_%H%M%S').log"
