...
export MASTER_PORT="${MASTER_PORT:-6035}"
...
if [[ ! -f "${AUDIO_PATH}" ]]; then
  ...
  echo "Edit AUDIO_PATH near the top of this script before running run_TA2V.sh." >&2
  exit 1
fi
...
$LAUNCH_PREFIX torchrun ${DISTRIBUTED_ARGS} inference/pipeline/entry.py ${MAGI_COMPILER_OFFLOAD_ARGS} \
  ...
  --sr_width "${SR_WIDTH}" \
  --sr_height "${SR_HEIGHT}" \
  --output_path "output_example_sr_1080p_ta2v_${BR_WIDTH}x${BR_HEIGHT}_${SR_WIDTH}x${SR_HEIGHT}_$(date '+%Y%m%d_%H%M%S')" \
  2>&1 | tee "log_example_sr_1080p_ta2v_${BR_WIDTH}x${BR_HEIGHT}_${SR_WIDTH}x${SR_HEIGHT}_$(date '+%Y%m%d_%H%M%S').log"
