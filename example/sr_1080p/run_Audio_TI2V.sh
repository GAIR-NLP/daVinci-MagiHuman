...
PROMPT_PATH="${PROJECT_ROOT}/example/assets/prompt_audio_ti2v_1080p.txt"
IMAGE_PATH="${PROJECT_ROOT}/example/assets/test_anchor_face.jpg"
AUDIO_PATH="${PROJECT_ROOT}/example/assets/test_english_voice.ogg"
CONFIG_PATH="${PROJECT_ROOT}/example/sr_1080p/config.json"
...
torchrun ${DISTRIBUTED_ARGS} inference/pipeline/entry.py \
  ...
  --sr_width 1920 \
  --sr_height 1088 \
  --output_path "output_example_sr_1080p_audio_ti2v_$(date '+%Y%m%d_%H%M%S')" \
  2>&1 | tee "log_example_sr_1080p_audio_ti2v_$(date '+%Y%m%d_%H%M%S').log"
